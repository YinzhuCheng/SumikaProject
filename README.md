...
- 世界记忆、用户记忆、好感度、主动发言、搜索、OCR/ASR 预留接口。
...
config/                  Persona and behavior configuration
deploy/                  Compose, systemd templates, deployment docs
scripts/                 Local maintenance and QQ operation scripts
...
`deploy/.env.example` and `deploy/napcat.env.example` show the required variable names without real credentials.
